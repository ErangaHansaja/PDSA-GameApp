...
from database.mincost import create_tables, save_round, update_overall_result, get_all_rounds, get_scoreboard
...
class MinCostGame:
    ...
    def _build_name_screen(self):
        ...
        tk.Frame(center, bg=BORDER, height=1).pack(fill=tk.X, pady=(S5, S3))

        self._make_btn(center, "  \u2190  Back to Dashboard  ", BG_INPUT, TEXT_SECONDARY,
                       self.window.destroy).pack(ipadx=S3, ipady=S1)
    ...
    def _show_round_result(self, is_correct, guess):
        ...
        if self.round_number < TOTAL_ROUNDS:
            ...
            btn_row = tk.Frame(res, bg=BG_DARK)
            btn_row.pack(anchor=tk.CENTER, pady=(S1, 0))
            self._make_btn(btn_row, "  View Results  \u2192  ", ACCENT_GREEN, BG_DARK,
                           self._show_summary).pack(side=tk.LEFT, ipadx=S5, ipady=S1)
    ...
    def _show_summary(self):
        ...
        action_row = tk.Frame(summ, bg=BG_DARK)
        action_row.pack(pady=S4)

        self._make_btn(action_row, "  \U0001f3c6  Scoreboard  ", ACCENT_ORANGE, BG_DARK,
                       self._show_scoreboard).pack(side=tk.LEFT, ipadx=S5, ipady=S1, padx=(0, S3))

        self._make_btn(action_row, "  Back to Dashboard  ", "#455a64", TEXT_PRIMARY,
                       self.window.destroy).pack(side=tk.LEFT, ipadx=S5, ipady=S1)
    # Screen 4: Scoreboard
    def _show_scoreboard(self):
        sb = tk.Toplevel(self.window)
        sb.title("Scoreboard \u2014 Minimum Cost Game")
        sb.geometry("1100x800")
        sb.resizable(False, False)
        sb.configure(bg=BG_DARK)
        sb.grab_set()

        hdr = tk.Frame(sb, bg=BG_SURFACE, height=56)
        hdr.pack(fill=tk.X)
        hdr.pack_propagate(False)
        tk.Label(hdr, text="\U0001f3c6  SCOREBOARD", font=FONT_DISPLAY,
                 bg=BG_SURFACE, fg=ACCENT_ORANGE).pack(side=tk.LEFT, padx=S6, pady=S3)
        tk.Frame(sb, bg=ACCENT_ORANGE, height=2).pack(fill=tk.X)

        table_wrap = tk.Frame(sb, bg=BG_DARK)
        table_wrap.pack(fill=tk.BOTH, expand=True, padx=S6, pady=(0, S2))
        ...
        style.configure("SB.Treeview.Heading",
                        background=BG_INPUT, foreground=ACCENT_ORANGE,
                        font=FONT_CAPTION_B, relief=tk.FLAT, borderwidth=0)
        ...
        cols = ("Rank", "Player", "Wins / 20", "Win %", "Date Played")
        tree = ttk.Treeview(table_wrap, columns=cols, show="headings",
                            height=12, style="SB.Treeview")

        widths = [60, 200, 100, 90, 110]
        anchors = [tk.CENTER, tk.W, tk.CENTER, tk.CENTER, tk.CENTER]
        ...
        for rank, row in enumerate(rows, start=1):
            player_name, wins, total_rounds, win_pct, played_at = row
            rank_display = f"{medals.get(rank, rank)}"
            tree.insert("", tk.END, values=(
                rank_display,
                player_name,
                f"{int(wins)} / {int(total_rounds)}",
                f"{win_pct}%",
                played_at,
            ))
        ...
        if not rows:
            tk.Label(table_wrap,
                     text="No games played yet. Be the first!",
                     font=FONT_BODY, bg=BG_DARK, fg=TEXT_MUTED).pack(expand=True)

        self._make_btn(sb, "  Close  ", "#455a64", TEXT_PRIMARY,
                       sb.destroy).pack(pady=S4)
