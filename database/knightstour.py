...
def init_db():
    try:
        ...
        c = conn.cursor()
        c.execute("""
            CREATE TABLE IF NOT EXISTS knightstour_winners (
        ...
        """)
        c.execute("""
            CREATE TABLE IF NOT EXISTS knightstour_algorithm_times (
                id              INTEGER PRIMARY KEY AUTOINCREMENT,
...
def save_winner(name: str, board_size: int, start_pos: tuple,
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO knightstour_winners (name, board_size, start_pos, moves, sequence, round, time_used) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (
                name,
                board_size,
...
def fetch_winners():
    ...
    try:
        ...
        c.execute(
            ...
            "FROM knightstour_winners ORDER BY id DESC LIMIT 50"
        )
        rows = c.fetchall()
        conn.close()
        return rows
...
def save_algorithm_time(player_name: str, round_number: int, algorithm_type: str,
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "INSERT INTO knightstour_algorithm_times (player_name, round_number, algorithm_type, board_size, time_taken_ms) "
            "VALUES (?, ?, ?, ?, ?)",
            (
                player_name,
                round_number,
    ...
    try:
        ...
        if player_name:
            c.execute(
                ...
                "FROM knightstour_algorithm_times WHERE player_name = ? ORDER BY id DESC LIMIT ?",
                (player_name, limit)
            )
        else:
            c.execute(
                "SELECT player_name, round_number, algorithm_type, board_size, time_taken_ms, timestamp "
                "FROM knightstour_algorithm_times ORDER BY id DESC LIMIT ?",
                (limit,)
            )
        rows = c.fetchall()
        conn.close()
    ...
    try:
        ...
        if player_name:
            c.execute(
                ...
                "FROM knightstour_algorithm_times WHERE player_name = ? GROUP BY algorithm_type, board_size",
                (player_name,)
            )
        else:
            c.execute(
                "SELECT algorithm_type, board_size, AVG(time_taken_ms) as avg_time, COUNT(*) as count "
                "FROM knightstour_algorithm_times GROUP BY algorithm_type, board_size"
            )
        rows = c.fetchall()
        conn.close()
